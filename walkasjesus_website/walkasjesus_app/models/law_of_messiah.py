...
class LawOfMessiah(models.Model):
    ...
    source = models.URLField(max_length=1024, blank=True, default='')
...
class LawOfMessiahBibleReference(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.get_reference_type_display()}: {self.get_book_display()} "
            f"{self.begin_chapter}:{self.begin_verse}-{self.end_chapter}:{self.end_verse}"
        )
